 ...
 * @package shopstar
 */
define( 'SHOPSTAR_THEME_VERSION' , '1.0.51' );
...
function shopstar_admin_notice() {
	...
	$response = wp_remote_get( 'http://www.outtheboxthemes.com/wp-json/wp/v2/themes/shopstar/' );
	...
	if( empty( $posts ) ) {
		...
		if ( !empty( $message ) && !get_user_meta( $user_id, 'shopstar_admin_notice_' .$message_id. '_dismissed' ) ) {
			$class = 'notice notice-success is-dismissible';
			printf( '<div class="%1$s"><p>%2$s</p><p><a href="?shopstar-admin-notice-dismissed&shopstar-admin-notice-id=%3$s">Dismiss this notice</a></p></div>', esc_attr( $class ), $message, $message_id );
		}
	}
}
...
function shopstar_wpforms_shareasale_id( $shareasale_id ) {
...
add_filter( 'wpforms_shareasale_id', 'shopstar_wpforms_shareasale_id' );
